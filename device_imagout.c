...
// uint32_t g_out_fmt = V4L2_PIX_FMT_RGB24;
...
static void fb_setup(void)
{
    ...
    if (ioctl(fd, MXCFB_SET_GBL_ALPHA, &alpha) < 0) {
		      err_msg("<%s>:set alpha %d failed for fb0\n",__func__,alpha.alpha);
    }
    if (ioctl(fd, FBIOGET_FSCREENINFO, &fb_fg_fix) < 0) {
            err_msg("%s:Get fix of fb2 failed\n",__func__);
            close(fd);
    }
    init_msg("%s:fb_fg_fix.id :%s \n",__func__,fb_fg_fix.id);
	close(fd);
}
...
static int  v4l_ouput_setup(struct v4l2_format *fmt)
{
        ...
        {
                err_msg("<%s>:set format failed\n",__func__);
                return TFAIL;
        }

        if (ioctl(g_fd_v4louput, VIDIOC_G_FMT, fmt) < 0)
        {
                err_msg("<%s>:get format failed\n",__func__);
                return TFAIL;
        }
        char fourcc[5]={0};
        strncpy(fourcc, (char *)&fmt->fmt.pix.pixelformat, 4);
        init_msg("%s:fmt: fmt.pix.width=%d"
                       "fmt.pix.height=%d "
                       "fmt.pix.pixelformat =%s\n",__func__,fmt->fmt.pix.width,fmt->fmt.pix.height,fourcc);
        ...
        {
                err_msg("%s:request buffers failed\n",__func__);
                return TFAIL;
        }
        g_num_buffers = buf_req.count;
        init_msg("%s:v4l2_output test: Allocated %d buffers\n",__func__, buf_req.count);
...
int  v4l_output_init(CAPTURE_IMAGOUT_INFO *info)
{
	...
	if (!ioctl(g_fd_v4louput, VIDIOC_QUERYCAP, &cap)) {
		init_msg("<%s>: %s :driver=%s, card=%s, bus=%s, "
				"version=0x%08x, "
				"capabilities=0x%08x\n",__func__,g_v4l_output,
    ...
    {
            err_msg("<%s>:get crop capability failed\n",__func__);
            retval = TFAIL;
            goto err;
    }
    
    g_display_left =(cropcap.bounds.width-g_display_width)/2;
    g_display_top  =(cropcap.bounds.height-g_display_height)/2;
    ...
    {
        err_msg("<%s>:set ctrl rotate failed\n",__func__);
        retval = TFAIL;
        goto err;
    }
    ...
    if (ioctl(g_fd_v4louput, VIDIOC_S_CTRL, &ctrl) < 0)
    {
        err_msg("<%s>:set ctrl vflip failed\n",__func__);
        retval = TFAIL;
        goto err;
    }
    ...
    if (ioctl(g_fd_v4louput, VIDIOC_S_CTRL, &ctrl) < 0)
    {
        err_msg("<%s>:set ctrl hflip failed\n",__func__);
        retval = TFAIL;
        goto err;
    }
	if (g_vdi_enable) {
		...
		if (ioctl(g_fd_v4louput, VIDIOC_S_CTRL, &ctrl) < 0)
		{
			err_msg("<%s>:set ctrl motion failed\n",__func__);
			 retval = TFAIL;
			 goto err;
		}
	}
    ...
    init_msg("<%s>:init success \n",__func__);
    return retval;     
err:
    return -1;
}

static int g_index=0; //dengerous!!!
int v4l_output_process(unsigned char * img,int len)
{
	...
	memset(&buf, 0, sizeof(buf));
        ...
        buf.type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
		buf.memory = V4L2_MEMORY_MMAP;
   		if (g_index < g_num_buffers) {
       		 buf.index = g_index;
        	if (ioctl(g_fd_v4louput, VIDIOC_QUERYBUF, &buf) < 0)
        	{
                err_msg("<%s>: VIDIOC_QUERYBUF failed\n",__func__);
				retval = -1;
        	}
            g_index++;
    	}
    	...
    	if ((start==0)&&(g_index == streamon_cnt)) { // Start playback after buffers queued
            type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
            if (ioctl (g_fd_v4louput, VIDIOC_STREAMON, &type) < 0) {
                   err_msg("<%s>:Could not start stream\n",__func__);
				   retval = -1;
            }
            start=1;
            fb_setup();
