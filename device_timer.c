...
int register_timer(const char *name,int  hz)
{
	int i=0;
...
{	
	...
	int i=0;
	static long long  st=0,last=0;
 	if(signo==SIGALRM){
 		//printf("[debug] capture_signal 1\n");
		for(i=0;i<MAX_NUM_MEMBER;i++){
    	...
    	g_timevalue.it_value.tv_usec = 1000000/g_timerinfo.basefreq;
    	g_timevalue.it_interval.tv_sec = 0;
    	g_timevalue.it_interval.tv_usec = 1000000/g_timerinfo.basefreq;
