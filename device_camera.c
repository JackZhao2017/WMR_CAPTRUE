...
int capture_getframedata(unsigned char *framebuf,int *len)
{
        ...
        endTime=currenttime();      
        memcpy((void *)framebuf,buffers[buf.index].start,buffers[buf.index].length);
...
static void print_pixelformat(void  *prefix, int val)
{
...
start:
        for (i = 0; i < TEST_BUFFER_NUM; i++)
        {
                ...
                buffers[i].start =(unsigned char *) mmap (NULL, buffers[i].length,PROT_READ | PROT_WRITE, MAP_SHARED,fd_v4l, 
                        buffers[i].offset);
                // printf("<%s>:Length: %d Address: %p \n",__func__, buf.length, buffers[i].start);
...
int capture_init(int out_width,int out_height,int cap_fmt,const char * dev)
{
	struct v4l2_format fmt;
